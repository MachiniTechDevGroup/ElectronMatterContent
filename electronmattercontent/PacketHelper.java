...
public class PacketHelper {
	...
	public static Packet createTileEntityPacket(String channel, String id, TileEntity te) {
	...
	public static Packet createCondenserPacket (TileEntityMatterCondenser te) {
		ByteArrayOutputStream bos = new ByteArrayOutputStream(140);
		DataOutputStream dos = new DataOutputStream(bos);
		try {
			dos.writeInt(te.xCoord);
			dos.writeInt(te.yCoord);
			dos.writeInt(te.zCoord);
			dos.writeInt(te.progress);
		} catch (Exception e) {
			EMC.warn(e.getMessage());
		}
		
		Packet250CustomPayload pkt = new Packet250CustomPayload();
		pkt.channel = "CondenserSync";
		pkt.data = bos.toByteArray();
		pkt.length = bos.size();
		return pkt;
	}
	
	public static Packet createCondenserPacket (ContainerMatterCondenser c) {
		Packet105UpdateProgressbar pkt = new Packet105UpdateProgressbar(c.windowId, 0, c.te.progress);
		return pkt;
	}
	...
	public static TileEntity handleTileEntityPacket(boolean readId, DataInputStream dis) {
		...
		if (world == null) {
			EMC.warn("World is... null?");
			return null;
		}
		TileEntity te = world.getBlockTileEntity(x, y, z);
		if (te == null) {
			EMC.warn("TileEntity is... null?");
			return null;
		}
		te.readFromNBT(tags);
		return te;
